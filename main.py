...
def youtube_scanner():
    ...
    return env.get_template('youtube-scanner.html').render(positive_result=positive_result,neutral_result=neutral_result,negative_result=negative_result,total_emotions=total_emotions)
...
def file_scanner():
    if request.method == "POST":
        ...
        sentiment,emotion = file_scan(mode,file)
    ...
    return env.get_template('file-scanner.html').render(sentiment=sentiment,emotion=emotion,dominant_emotion=dominant_emotion)
...
def analyse_youtube(id,maxResults):
    ...
    return (positive,neutral,negative,total_emotions)
...
def scan_file_text(file,text):
    ...
    file.close()
    os.remove(file.name)
    return sentiment,total_emotions

def file_scan(mode,file):
    if mode == "single":
        with open(f"{file_uploads}/{file.filename}","r") as file:
            text = file.read()
            sentiment, total_emotions = scan_file_text(file,text)
            return sentiment, total_emotions
    elif mode == "multiple":
        with open(f"{file_uploads}/{file.filename}","r") as file:
            text = file.read()
